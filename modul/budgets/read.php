...
$sql = $conn->prepare("SELECT
            ...
            c.nama_kategori
        FROM budgets b
        JOIN categories c ON b.id_kategori = c.id_kategori
        WHERE b.id_pengguna = ?
        ORDER BY b.tahun DESC, b.bulan DESC, c.nama_kategori ASC");
if (!$sql) {
...
<body>
    ...
    <div class="dashboard-wrapper">
        ...
        <main class="main-content">
            ...
            <div class="widget-card"> <div class="content-header">
                    <h1>Manajemen Anggaran Bulanan 📊</h1>
                    <a href="create.php" class="btn btn-success">➕ Buat Anggaran Baru</a>
                </div>
                    ...
                    <tbody>
                        ...
                        if ($result && $result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo "<td>Rp " . number_format($row['jumlah_anggaran'], 0, ',', '.') . "</td>";
                                echo "<td>" . $row['bulan'] . "/" . $row['tahun'] . "</td>";
                                echo "<td>
                                        <a href='update.php?id=" . $row['id_anggaran'] . "' class='btn btn-primary action-link'>Edit</a>
                                        <a href='process.php?action=delete&id=" . $row['id_anggaran'] . "' class='btn btn-danger action-link' onclick=\"return confirm('Yakin ingin menghapus anggaran ini?')\">Hapus</a>
                                      </td>";
                                echo "</tr>";
                            }
                        } else {
                            echo "<tr><td colspan='4' style='text-align: center; padding: 20px;'>Anda belum memiliki data anggaran.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
