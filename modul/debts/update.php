...
$id = $_GET['id'] ?? null;
if (!$id || !is_numeric($id)) {
    header('Location: read.php?message=ID tidak valid&status=error');
    exit();
}


$stmt = $conn->prepare("SELECT * FROM debts WHERE id_utang = ? AND id_pengguna = ?");
$stmt->bind_param("ii", $id, $current_user_id);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows === 0) {
    header('Location: read.php?message=Data tidak ditemukan atau akses ditolak&status=error');
    exit();
}
    ...
    <div class="dashboard-wrapper">
        <aside class="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="read.php" class="active">Utang (Debts)</a> <a href="../receivables/read.php">Piutang (Receivables)</a>
                <h2>Fitur Lain</h2>
        ...
        <main class="main-content">
            <div class="widget-card">
                <h1>Edit Data Utang ✏️</h1>
                <p style="margin-top: -20px; margin-bottom: 20px;"><a href="read.php" class="btn btn-primary"> &lt;-- Kembali ke Daftar</a></p>

                <form action="process.php" method="POST">
                    <input type="hidden" name="action" value="update">
                    <input type="hidden" name="id_utang" value="<?= $debt['id_utang'] ?>">
                    <input type="hidden" name="id_pengguna" value="<?= $debt['id_pengguna'] ?>">

                    <div class="form-group">
                        <label for="jumlah_utang">Jumlah Utang (Rp):</label>
                        <input type="number" name="jumlah_utang" id="jumlah_utang" step="1" value="<?= $debt['jumlah_utang'] ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="tanggal_tenggat">Tanggal Tenggat (Jatuh Tempo):</label>
                        <input type="date" name="tanggal_tenggat" id="tanggal_tenggat" value="<?= $debt['tanggal_tenggat'] ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="status_utang">Status Utang:</label>
                        <select name="status_utang" id="status_utang" required>
                            <option value="Belum Dibayar" <?= ($debt['status_utang'] == 'Belum Dibayar') ? 'selected' : '' ?>>Belum Dibayar</option>
                            <option value="Dibayar Sebagian" <?= ($debt['status_utang'] == 'Dibayar Sebagian') ? 'selected' : '' ?>>Dibayar Sebagian</option>
                            <option value="Lunas" <?= ($debt['status_utang'] == 'Lunas') ? 'selected' : '' ?>>Lunas</option>
                        </select>
                    </div>
