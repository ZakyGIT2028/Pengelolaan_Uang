...
$sql = $conn->prepare("SELECT * FROM debts WHERE id_pengguna = ? ORDER BY status_utang ASC, tanggal_tenggat ASC");
...
<head>
    ...
    <title>Manajemen Utang - FinSet</title>
        ...
        <main class="main-content">
            ...
            <div class="widget-card">
                ...
                <div class="content-header">
                    <h1>Daftar Utang Anda 📉</h1>
                    <a href="create.php" class="btn btn-success">➕ Catat Utang Baru</a>
                </div>
                    ...
                    <tbody>
                        ...
                        if ($result && $result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                $status_class = 'status-' . str_replace(' ', '-', $row['status_utang']);

                                echo "<tr>";
                                echo "<td>" . date('d M Y', strtotime($row['tanggal_tenggat'])) . "</td>";
                                echo "<td>Rp " . number_format($row['jumlah_utang'], 0, ',', '.') . "</td>";
                                echo "<td><span class='status-badge " . $status_class . "'>" . htmlspecialchars($row['status_utang']) . "</span></td>";
                                echo "<td>
                                        <a href='update.php?id=" . $row['id_utang'] . "' class='btn btn-primary action-link'>Edit</a>
                                        <a href='process.php?action=delete&id=" . $row['id_utang'] . "' class='btn btn-danger action-link' onclick=\"return confirm('Yakin ingin menghapus data utang ini?')\">Hapus</a>
                                      </td>";
                                echo "</tr>";
                            }
                        } else {
                            echo "<tr><td colspan='4' style='text-align:center; padding: 20px;'>Tidak ada catatan utang. Bagus!</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
