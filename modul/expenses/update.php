...
$id = $_GET['id'] ?? null;
if (!$id || !is_numeric($id)) {
    header('Location: read.php?message=ID Pengeluaran tidak valid&status=error');
    exit();
}

$stmt = $conn->prepare("SELECT e.*, c.nama_kategori
                        FROM expenses e
                        LEFT JOIN categories c ON e.id_kategori = c.id_kategori
                        WHERE e.id_pengeluaran = ? AND e.id_pengguna = ?");
...
if ($result_expense->num_rows === 0) {
    header('Location: read.php?message=Pengeluaran tidak ditemukan atau Anda tidak punya hak akses&status=error');
    exit();
}
$expense = $result_expense->fetch_assoc();
$stmt->close();
$current_category_name = $expense['nama_kategori'] ?? ''; 
...
<head>
    ...
    <title>Edit Pengeluaran - FinSet</title>
    ...
    <div class="dashboard-wrapper">
        ...
        <main class="main-content">
            ...
            <div class="widget-card">
                ...
                <h1>Edit Pengeluaran (ID: <?= $expense['id_pengeluaran'] ?>) ✏️</h1>
                ...
                <form action="process.php" method="POST">
                    ...
                    <input type="hidden" name="id_pengeluaran" value="<?= $expense['id_pengeluaran'] ?>">
                    <input type="hidden" name="id_pengguna" value="<?= $expense['id_pengguna'] ?>">

                     <div class="form-group">
                        <label for="tanggal">Tanggal:</label>
                        <input type="date" name="tanggal" id="tanggal" value="<?= htmlspecialchars($expense['tanggal']) ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="kategori_input">Kategori (Ketik nama baru jika belum ada):</label>
                        <input type="text" name="kategori_input" id="kategori_input" list="kategori_list"
                               value="<?= htmlspecialchars($current_category_name) ?>" required placeholder="Cth: Makanan, Transportasi">
                     ...
                     <div class="form-group">
                        ...
                        <input type="number" name="jumlah" id="jumlah" step="1" value="<?= $expense['jumlah'] ?>" required>
                    </div>
                        ...
                        <select name="id_metode" id="id_metode" required>
                             <?php if ($methods_result && $methods_result->num_rows > 0): ?>
                                ...
                                <?php while ($row = $methods_result->fetch_assoc()): ?>
                                    <option value="<?= $row['id_metode'] ?>" <?= ($expense['id_metode'] == $row['id_metode']) ? 'selected' : '' ?>>
                        ...
                        <select name="id_mata_uang" id="id_mata_uang" required>
                             <?php if ($currencies_result && $currencies_result->num_rows > 0): ?>
                                ...
                                <?php while ($row = $currencies_result->fetch_assoc()): ?>
                                    <option value="<?= $row['id_mata_uang'] ?>" <?= ($expense['id_mata_uang'] == $row['id_mata_uang']) ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($row['kode_mata_uang']) ?> - <?= htmlspecialchars($row['nama_mata_uang']) ?>
                                    </option>
                                <?php endwhile; ?>
                             <?php else: ?>
