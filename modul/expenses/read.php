...
$sql = $conn->prepare("SELECT
            ...
            cr.kode_mata_uang
        FROM expenses e
        LEFT JOIN categories c ON e.id_kategori = c.id_kategori /* LEFT JOIN in case category was deleted */
        LEFT JOIN payment_methods m ON e.id_metode = m.id_metode
        LEFT JOIN currencies cr ON e.id_mata_uang = cr.id_mata_uang
        WHERE e.id_pengguna = ?
        ORDER BY e.tanggal DESC, e.id_pengeluaran DESC");
...
<head>
    ...
    <title>Pengeluaran - FinSet</title>
        ...
        <main class="main-content">
            ...
            <div class="widget-card">
                ...
                <div class="content-header">
                    ...
                    <a href="create.php" class="btn btn-success">➕ Tambah Pengeluaran Baru</a>
                </div>
                    ...
                    <tbody>
                        ...
                        if ($result && $result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo "<td>" . htmlspecialchars($row['kode_mata_uang'] ?? 'N/A') . " " . number_format($row['jumlah'], 0, ',', '.') . "</td>";
                                echo "<td>" . ($row['nama_metode'] ? htmlspecialchars($row['nama_metode']) : '-') . "</td>";
                                echo "<td>" . ($row['kode_mata_uang'] ? htmlspecialchars($row['kode_mata_uang']) : '-') . "</td>";
                                echo "<td>
                                        <a href='update.php?id=" . $row['id_pengeluaran'] . "' class='btn btn-primary action-link'>Edit</a>
                                        <a href='process.php?action=delete&id=" . $row['id_pengeluaran'] . "' class='btn btn-danger action-link' onclick=\"return confirm('Yakin ingin menghapus data ini?')\">Hapus</a>
                                      </td>";
                                echo "</tr>";
                            }
                        } else {
                            echo "<tr><td colspan='6' style='text-align: center; padding: 20px;'>Anda belum memiliki data pengeluaran.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
