...
$stmt = $conn->prepare("SELECT * FROM receivables WHERE id_piutang = ? AND id_pengguna = ?");
...
<head>
    ...
    <title>Edit Piutang - FinSet</title>
    ...
    <div class="dashboard-wrapper">
        ...
        <main class="main-content">
            <div class="widget-card">
                <h1>Edit Data Piutang ✏️</h1>
                <p style="margin-top: -20px; margin-bottom: 20px;"><a href="read.php" class="btn btn-primary"> &lt;-- Kembali ke Daftar</a></p>

                <form action="process.php" method="POST">
                    <input type="hidden" name="action" value="update">
                    <input type="hidden" name="id_piutang" value="<?= $receivable['id_piutang'] ?>">
                    <input type="hidden" name="id_pengguna" value="<?= $receivable['id_pengguna'] ?>">

                    <div class="form-group">
                        <label for="jumlah_piutang">Jumlah Piutang (Rp):</label>
                        <input type="number" name="jumlah_piutang" id="jumlah_piutang" step="1" value="<?= $receivable['jumlah_piutang'] ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="tanggal_tenggat">Tanggal Tenggat (Jatuh Tempo):</label>
                        <input type="date" name="tanggal_tenggat" id="tanggal_tenggat" value="<?= $receivable['tanggal_tenggat'] ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="status_piutang">Status Piutang:</label>
                        <select name="status_piutang" id="status_piutang" required>
                            <option value="Belum Diterima" <?= ($receivable['status_piutang'] == 'Belum Diterima') ? 'selected' : '' ?>>Belum Diterima</option>
                            <option value="Diterima Sebagian" <?= ($receivable['status_piutang'] == 'Diterima Sebagian') ? 'selected' : '' ?>>Diterima Sebagian</option>
                            <option value="Lunas" <?= ($receivable['status_piutang'] == 'Lunas') ? 'selected' : '' ?>>Lunas</option>
                        </select>
                    </div>
