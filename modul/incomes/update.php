...
$id = $_GET['id'] ?? null;
if (!$id || !is_numeric($id)) {
    header('Location: read.php?message=ID Pemasukan tidak valid&status=error');
    exit();
}

$stmt = $conn->prepare("SELECT * FROM incomes WHERE id_pemasukan = ? AND id_pengguna = ?");
...
if ($result_income->num_rows === 0) {
    header('Location: read.php?message=Pemasukan tidak ditemukan atau Anda tidak punya hak akses&status=error');
    exit();
}
...
<head>
    ...
    <title>Edit Pemasukan - FinSet</title>
    ...
    <div class="dashboard-wrapper">
        ...
        <main class="main-content">
            ...
            <div class="widget-card">
                ...
                <h1>Edit Pemasukan (ID: <?= $income['id_pemasukan'] ?>) ✏️</h1>
                ...
                <form action="process.php" method="POST">
                    ...
                    <input type="hidden" name="id_pemasukan" value="<?= $income['id_pemasukan'] ?>">
                    <input type="hidden" name="id_pengguna" value="<?= $income['id_pengguna'] ?>">

                     <div class="form-group">
                        <label for="tanggal">Tanggal:</label>
                        <input type="date" name="tanggal" id="tanggal" value="<?= htmlspecialchars($income['tanggal']) ?>" required>
                    </div>

                     <div class="form-group">
                        <label for="sumber">Sumber Pemasukan:</label>
                        <input type="text" name="sumber" id="sumber" value="<?= htmlspecialchars($income['sumber']) ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="jumlah">Jumlah:</label>
                        <input type="number" name="jumlah" id="jumlah" step="1" value="<?= $income['jumlah'] ?>" required>
                    </div>
                        ...
                        <select name="id_metode" id="id_metode" required>
                             <?php if ($methods_result->num_rows > 0): ?>
                                <?php while ($row = $methods_result->fetch_assoc()): ?>
                                    <option value="<?= $row['id_metode'] ?>" <?= ($income['id_metode'] == $row['id_metode']) ? 'selected' : '' ?>>
                        ...
                        <select name="id_mata_uang" id="id_mata_uang" required>
                             <?php if ($currencies_result->num_rows > 0): ?>
                                <?php while ($row = $currencies_result->fetch_assoc()): ?>
                                    <option value="<?= $row['id_mata_uang'] ?>" <?= ($income['id_mata_uang'] == $row['id_mata_uang']) ? 'selected' : '' ?>>
