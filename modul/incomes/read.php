...
$sql = $conn->prepare("SELECT
            ...
            cr.kode_mata_uang
        FROM incomes i
        JOIN payment_methods m ON i.id_metode = m.id_metode
        JOIN currencies cr ON i.id_mata_uang = cr.id_mata_uang
        WHERE i.id_pengguna = ?
        ...
        <main class="main-content">
            ...
            <div class="widget-card">
                ...
                <div class="content-header">
                    ...
                    <a href="create.php" class="btn btn-success">➕ Tambah Pemasukan Baru</a>
                </div>
                    ...
                    <tbody>
                        ...
                        if ($result && $result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo "<td>" . htmlspecialchars($row['kode_mata_uang']) . " " . number_format($row['jumlah'], 0, ',', '.') . "</td>";
                                echo "<td>" . htmlspecialchars($row['nama_metode']) . "</td>";
                                echo "<td>" . htmlspecialchars($row['kode_mata_uang']) . "</td>";
                                echo "<td>
                                        <a href='update.php?id=" . $row['id_pemasukan'] . "' class='btn btn-primary action-link'>Edit</a>
                                        <a href='process.php?action=delete&id=" . $row['id_pemasukan'] . "' class='btn btn-danger action-link' onclick=\"return confirm('Yakin ingin menghapus data ini?')\">Hapus</a>
                                      </td>";
                                echo "</tr>";
                            }
                        } else {
                            echo "<tr><td colspan='6' style='text-align: center; padding: 20px;'>Anda belum memiliki data pemasukan.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
